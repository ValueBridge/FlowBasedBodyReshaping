...
def build_app_container(context):
    ...
    # Compute path to requirements file based on operating system
    import platform

    requirements_path = "requirements.txt" if platform.system() == "Linux" else "requirements_osx.txt"

    command = (
        "DOCKER_BUILDKIT=1 docker build "
        f"--build-arg REQUIREMENTS_FILE={requirements_path} "
        "--tag photobridge/flow_based_body_reshaping:latest "
